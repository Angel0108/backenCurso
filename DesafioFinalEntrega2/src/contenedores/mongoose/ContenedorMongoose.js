...
class ContenedorMongoose {
    ...
    async getById(id) {
        const ret = await this.model.getById(id)
    }

    async getAll() {
        const ret = await this.model.getAll()
    }

    async insert(element) {
        console.log('Try to Insert this element : ' + element)
        await this.model.create(element);
        console.log('Insert ok this element : ' + element)
    }

    async deleteById(id) {
        await mongoose.deleteById(id)
    }
    ...
    async createSchema(element) {
        await mongoose.createSchema(element)
    }

    async getById(element) {
        return await mongoose.getById(element.id)
    }

    async getAll() {
        return await mongoose.getAll()
    }
...
/*ContenedorMongoose.connect()
ContenedorMongoose.insert(estudiantes[0])

ContenedorMongoose.disconnect()*/
